...
// Fit to the DATA's footprint (not the whole US) so the dots fill the frame.
// Outline states that fall entirely outside this box (e.g. AK, HI) are dropped.
const MAP_W = 640, MAP_H = 420, PAD = 16;
const lats = DATA.map(d=>d.lat), lons = DATA.map(d=>d.lon);
const M = 1.5; // degrees of margin so border states show for context
const latMin=Math.min(...lats)-M, latMax=Math.max(...lats)+M;
const lonMin=Math.min(...lons)-M, lonMax=Math.max(...lons)+M;
...
// state polygons — only those overlapping the data box (drops AK, HI, far-west states)
function bboxOverlaps(geom){
  let lo1=Infinity,la1=Infinity,lo2=-Infinity,la2=-Infinity;
  eachCoord(geom,(lon,lat)=>{ if(lon<lo1)lo1=lon; if(lat<la1)la1=lat; if(lon>lo2)lo2=lon; if(lat>la2)la2=lat; });
  return !(lo2<lonMin || lo1>lonMax || la2<latMin || la1>latMax);
}
if (OUTLINE){
  OUTLINE.features.forEach(f=>{
    if(!bboxOverlaps(f.geometry)) return;
